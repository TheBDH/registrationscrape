...
for department in open('departments'):
	...
	while (num_classes >= 24):
		...
		while (start != -1) and (end != -1):
			start = resp.find("Show_Detail('201520','") + 22
			end = resp.find("','", start)
...
print 'REMOVING ' + str(len(independent_studies)) + ' INDEPENDENT STUDIES'
for CRN in independent_studies:
	if CRN in CRNs:
		CRNs.remove(CRN)

print 'REMOVING ' + str(len(above_3000)) + ' CLASSES ABOVE 3000'
for CRN in above_3000:
	if CRN in CRNs:
		CRNs.remove(CRN)

print 'REMOVING ' + str(len(conferences)) + ' CONFERENCES'
for CRN in conferences:
	if CRN in CRNs:
		CRNs.remove(CRN)

print 'CRNS length: ' + str(len(CRNs))
